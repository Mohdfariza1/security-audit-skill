...
function readFileWithinLimit(file) {
  ...
  try {
    if (isWindows) {
      // Windows has no O_NOFOLLOW/O_NONBLOCK; reject symlinks via lstat instead.
      if (fs.lstatSync(file).isSymbolicLink()) throw new SafeInputError("input must not be a symlink");
